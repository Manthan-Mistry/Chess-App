...
def display_player_stats(df: pd.DataFrame, player: str) -> Tuple[int, float, float, int, int, int, int, float, float, float,
    ...
    white_accuracy = round(df[df['white_username'] == (player)]['white_accuracy'].mean(),2)
    black_accuracy = round(df[df['black_username'] == (player)]['black_accuracy'].mean(),2)
...
def  show_white_stats(total_games_white: int, white_win_ratio: float, white_draw_ratio: float, 
    ...
    with st.container(height = 375):
        ...
        with col1:
            ...
            # Horizontal Chart:
            # fig = create_horizontal_stacked_bar_chart(white_win_ratio, white_draw_ratio, white_loss_ratio, wins_as_white, draws_as_white, loss_as_white, height=60, width=200)
            # st.plotly_chart(fig, config={'displayModeBar': False, 'responsive': True})

        with col2:
            st.markdown(f"""
                <div class="metric-container-spec">
                    <div class="metric-label">◻ Total Win</div>
...
def show_black_stats(total_games_black: int, black_win_ratio: float, black_draw_ratio: float, 
    ...
    with st.container(height = 375):
            ...
            with col1:
                ...
                """, unsafe_allow_html=True)
                st.write('')
                # fig = create_horizontal_stacked_bar_chart(black_win_ratio, black_draw_ratio, black_loss_ratio, wins_as_black, draws_as_black, loss_as_black, height = 60, width = 200)
                # st.plotly_chart(fig, theme= 'streamlit', config={'displayModeBar': False, 'responsive': False})
